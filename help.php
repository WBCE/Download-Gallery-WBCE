...
<div class="download_gallery_help">
	...
	<ul>
		...
		<li><strong>Suche aktivieren</strong> - Schaltet die Filterfuktion im Frontend ein/aus.</li>
	...
	<ul>
	<li><code>$DGTEXT[...]</code> Aufrufe: Sprachspezifische Ausgaben. Diese können in den Sprachdateien, also z.B. in in der /modules/download_gallery/languages/de.php angepasst werden.</li>
	<li><code>if($data->settings[...]</code> Aufrufe: Prüft, welche Optionen im DLG3-Backend vorgenommen wurden. (Allgemeine Info: Bei einer if-Anweisung in dieser Templatesprache wird die Bedingung in runden Klammern angegegeben. Wo in anderen Sprachen wie PHP oder JavaScript eine geschweifte öffnende Klammer gesetzt wird, wird hier ein Doppelpunkt verwendet. Jede if-Anweisung muss nach dem bei erfüllter Bedingung auszuführenden Code mit <code>&lt;?php endif; ?&gt;</code> beendet werden.)</li>
	...
	<li><code>if($data->filecount != $data->num_files):</code> &ndash;  Verstehe ich nicht. Ist aber sicherlich notwendig und sollte so belassen werden.</li>
	...
	<li><code>&lt;img src="&lt;?php echo WB_URL ?&gt;/modules/download_gallery/images/&lt;?php echo $image ?&gt;" alt="" /&gt;</code> &ndash;  Zeigt das zum Dateityp passende Icon an. Diese müssen bei Übernahme dieses Codes im Verzeichnis "images" unterhalb des DLG-Modulverzeichnisses sein.</li> 
	...
	<ul>
		...
		<li><strong>Search Filter</strong> - Whether or not the file list filter should be displayed.</li>
		...
		<li><strong>User upload</strong> - Who will be allowed to upload files from the frontend. If it is set to authorized users or public, you can also select if there should be a captcha protection. <br />Be extremely careful with this option! Usually you probably <strong>DON'T</strong> want to allow uploading files to the public!</li> 
	</ul><br />
	...
	To create or edit a frontend view, have a look at the module files (access via FTP or the <a href="http://addons.wbce.org/?do=item&item=28" target="_blank">AddonFile Editor</a>). As you can see, there is a new directory <code>templates/default</code> inside the download_gallery folder, and this directory contains the subfolders named <code>backend</code> and <code>frontend</code>. In the frontend you find another folder, named <code>tableview</code> which is the default frontend style.</p>
	...
	<ul>
	<li><code>$DGTEXT[...]</code> stuff: The language specific outputs. You can edit them in the language files of the DLG3, e.g. /modules/download_gallery/languages/de.php and so on.</li>
	...
	<li><code>if($data->filecount != $data->num_files):</code> &ndash;  I have no idea what this means, sorry. Probably it's important and should not be changed or removed.</li>
	...
	<li><code>&lt;img src="&lt;?php echo WB_URL ?&gt;/modules/download_gallery/images/&lt;?php echo $image ?&gt;" alt="" /&gt;</code> &ndash;  Show the corresponding image to the file type. They are expected to be in the images directory of the DLG3. Change this according to your needs if you have nicer images stored anywhere else.</li> 
