...
// Add column to table
$mod_dlg = "SELECT `released` FROM `".TABLE_PREFIX."mod_download_gallery_files` LIMIT 0";
if (false == $database->query($mod_dlg)) {
	//add the column
	$mod_dlg = 'ALTER TABLE `'.TABLE_PREFIX.'mod_download_gallery_files` ADD `released` INT NOT NULL DEFAULT 0';
	$database->query($mod_dlg);
}
