...
function dlg_ext2img($section_id) : array
{
    global $database, $tablename;
    $data       = [];
    $query_ext  = $database->query(sprintf(
        "SELECT `file_image`,`extensions` FROM `%s%s_file_ext` WHERE `section_id`=%d",
            TABLE_PREFIX, $tablename, intval($section_id)
    ));
